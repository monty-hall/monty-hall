  ...
  render() {
    return (
      <div id="menu" ref="menu">
        ...
        <a href="#" onClick={this.props.logOff}>
          {" "}
          End Session{" "}
        </a>
        {this.props.sandbox && (
          <a href="#" onClick={this.handleClickSandbox}>
            Sandbox
          </a>
        )}
        <a href="#" onClick={this.handleClickPlay}>
          Play
        </a>
      </div>
    );
...
class App extends React.Component {
  ...
  getSession = () => {
    axios.get("http://127.0.0.1:5000/new_session").then(
      (response) => {
  ...
  updateMode = (mde) => {
    this.setState({ mode: mde });
  };
  updateDoors = (d) => {
    this.setState({ doors: d });
  };
  updateDoor1 = (d) => {
  ...
  toSandboxMode = () => {
    ...
    window.localStorage.setItem(
      "user",
      JSON.stringify({ session: this.state.session, sandboxAvailable: true })
    );
  };
  toSandboxScreen = () => {
    this.setState({
  ...
  autoPlay = (count) => {
    axios.post("http://127.0.0.1:5000/auto", {
      sess: this.state.session,
      monty: this.state.mode,
      doors: this.state.doors,
      count: count,
  ...
  render() {
      ...
      if (res == null) {
        ...
        if (stored == null || stored.session == null) {
          ...
          this.setState({
            ...
            sandboxAvailable: stored.sandboxAvailable,
          });
        }
      }
    }
    if (
      (this.state.sandboxAvailable === false ||
        this.state.text === "Standard") &&
    ...
    if (
      this.state.sandboxAvailable === false &&
      this.state.door_1 != null &&
      this.state.door_2 != null
    ) {
      display = (
        <Result
          selected={this.state.door_2}
          correct={this.state.winning_door}
          resetDoors={this.resetDoors}
          toPlayScreen={this.toSandboxMode}
          t={"Finish and play sandbox mode"}
        />
      );
      ...
      display = (
        <Sandbox
          updateDisplayCB={this.updateDisplay}
          updateMystery={this.updateMystery}
          updateMonty={this.updateMode}
          updateDoors={this.updateDoors}
          updateWinningDoor={this.updateWinner}
        />
      );
    } else if (
      this.state.text === "Standard" &&
      ...
      display = (
        <Result
          ...
          correct={this.state.winning_door}
          resetDoors={this.resetDoors}
          toPlayScreen={this.toSandboxScreen}
          t={"New Game"}
        />
      );
    ...
    return (
      <div>
        <Menu
          updateDisplayCB={this.updateDisplay}
          sandbox={this.state.sandboxAvailable}
          logOff={this.logOff}
        />
        ,{display}
      </div>
